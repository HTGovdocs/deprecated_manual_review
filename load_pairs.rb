...
@insert_pair_sql = "INSERT INTO mr_pairs (first_id, second_id, relationship, score) 
                    VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE relationship=? and score=?"
@insert_pair = @conn.prepare(@insert_pair_sql)
open(ARGV[0]).each do |line|
  rel, score, gd_id_list = line.chomp.split(/\t/)
  ...
  gd_ids.combination(2).each do | pair |
    @insert_pair.execute(pair[0], pair[1], rel, score, rel, score)
  end
end
